...
class OLEModule
{
    public:
		...
		OLEModule(pcl::PointCloud<PointType>::Ptr cloud,pcl::search::KdTree<PointType>::Ptr kdtree=NULL);
};
...
double OLEModule::GetMinorEval(int K,string str)
{
	...
	if("Common"==str){
		...
		for(int i=0;i<cloud_->points.size();i++){
			double meval_tmp=GetMEval(cloud_,40,kdtree_,i);
		...
		for(int i=0;i<n_;i++){
			int itmp=rand()%N_;
			double meval_tmp=GetMEval(cloud_,40,kdtree_,itmp);
			double dmean_tmp=ComputeNearestDistance(kdtree_,itmp);
